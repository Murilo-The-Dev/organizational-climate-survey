...
type UsuarioAdministradorCreateRequest struct {
	...
	NomeAdmin string `json:"nome_admin" binding:"required,min=2,max=255" example:"Joao Silva"`       // Nome completo do administrador
	Email     string `json:"email" binding:"required,email,max=255" example:"joao@empresa.com"`      // E-mail válido do administrador
	Senha     string `json:"senha" binding:"required,min=8,max=128" example:"SenhaForte@123"`        // Senha em texto plano (antes do hash)
	Status    string `json:"status" binding:"required,oneof=Ativo Inativo Pendente" example:"Ativo"` // Estado atual do usuário no sistema
}
...
type UsuarioAdministradorUpdateRequest struct {
	NomeAdmin *string `json:"nome_admin,omitempty" binding:"omitempty,min=2,max=255" example:"Maria Oliveira"`     // Novo nome do administrador (opcional)
	Email     *string `json:"email,omitempty" binding:"omitempty,email,max=255" example:"maria@empresa.com"`       // Novo e-mail (opcional)
	Status    *string `json:"status,omitempty" binding:"omitempty,oneof=Ativo Inativo Pendente" example:"Inativo"` // Novo status (opcional)
}
...
type UsuarioAdministradorLoginRequest struct {
	Email string `json:"email" binding:"required,email" example:"admin@empresa.com"` // E-mail de login
	Senha string `json:"senha" binding:"required" example:"SenhaForte@123"`          // Senha em texto plano
}
